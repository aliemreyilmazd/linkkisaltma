...
<div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
    ...
    <a class="dropdown-item d-flex align-items-center" href="{{ $link->url }}" target="_blank" rel="nofollow noreferrer noopener">@include('icons.open-new', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Open') }}</a>
