@section('site_title', formatTitle([__('Captcha'), config('settings.title')]))
...
<h2 class="mb-3 d-inline-block">{{ __('Captcha') }}</h2>
...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.captcha') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="i-captcha-shorten">{{ Str::ucfirst(mb_strtolower(__(':name form', ['name' => __('Shorten')]))) }}</label>
                <select name="captcha_shorten" id="i-captcha-shorten" class="custom-select{{ $errors->has('captcha_shorten') ? ' is-invalid' : '' }}">
                    @foreach([0 => __('Disabled'), 1 => __('Enabled')] as $key => $value)
                        <option value="{{ $key }}" @if ((old('captcha_shorten') !== null && old('captcha_shorten') == $key) || (config('settings.captcha_shorten') == $key && old('captcha_shorten') == null)) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                @if ($errors->has('captcha_shorten'))
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $errors->first('captcha_shorten') }}</strong>
                    </span>
                @endif
            </div>

            <div class="form-group">
                <label for="i-captcha-registration">{{ Str::ucfirst(mb_strtolower(__(':name form', ['name' => __('Registration')]))) }}</label>
            ...
            <div class="form-group">
                <label for="i-captcha-contact">{{ Str::ucfirst(mb_strtolower(__(':name form', ['name' => __('Contact')]))) }}</label>
