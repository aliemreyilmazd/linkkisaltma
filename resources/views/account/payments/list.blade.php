...
<div class="d-flex">
    <div class="flex-grow-1">
        <h2 class="mb-3 d-inline-block">{{ __('Payments') }}</h2>
    </div>
</div>
                            ...
                            <div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow width-64" id="search-filters">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-auto">
                                            @if(request()->input('search') || request()->input('type') || request()->input('status') || request()->input('sort'))
                                                <a href="{{ route('account.payments') }}" class="text-secondary">{{ __('Reset') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group px-4">
                                    ...
                                    <select name="by" id="i-by" class="custom-select custom-select-sm">
                                        @foreach(['payment' => __('Payment ID'), 'invoice' => __('Invoice ID')] as $key => $value)
                                            <option value="{{ $key }}" @if(request()->input('by') == $key || !request()->input('by') && $key == 'payment') selected @endif>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col text-truncate">
                                <div class="row align-items-center text-truncate">
                                    <div class="col-12 col-lg-8 d-flex">
                                        <div class="text-truncate">
                                            <div class="d-flex align-items-center">
                                                <img src="{{ url('/') }}/images/icons/payments/{{ $payment->processor }}.svg" class="width-6 rounded-sm">
                                                ...
                                                @if($payment->status == 'completed')
                                                    <a href="{{ route('account.invoices.show', $payment->id) }}" class="badge badge-secondary text-truncate" data-enable="tooltip" title="{{ __('Invoice') }}">{{ $payment->invoice_id }}</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-none d-lg-block col-lg-2">
                                        <div class="text-truncate" data-enable="tooltip" title="{{ $payment->created_at->tz(Auth::user()->timezone ?? config('app.timezone'))->format(__('Y-m-d') . ' H:i:s') }}">{{ $payment->created_at->diffForHumans() }}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-auto">
