...
class CheckoutController
{
    ...
    public function index(Request $request, $id)
    {
        ...
        $plan = Plan::where('id', '=', $id)->priced()->firstOrFail();
        ...
        if ($plan->coupons) {
            ...
            if ($request->old('coupon')) {
                ...
                if (!in_array($coupon->id, $plan->coupons)) {
                    $coupon = null;
                }
            }
        }
    ...
    private function tryPlan(Request $request, Plan $plan)
    {
        ...
        $request->user()->plan_subscription_id = null;
        $request->user()->plan_subscription_status = null;
        $request->user()->plan_created_at = Carbon::now();
        $request->user()->plan_recurring_at = null;
        $request->user()->plan_trial_ends_at = Carbon::now()->addDays($plan->trial_days);
        $request->user()->plan_ends_at = Carbon::now()->addDays($plan->trial_days);
        $request->user()->save();
